...
export default function BeautyAndSkin({handleButtonClick}) {
    const {medicineData,addToCart} = useContext(CartContext)
    const beautyAndSkin = medicineData[0].categoryData
  return (
    <div className='grid grid-cols-4 gap-4 ml-[2rem] mr-[2rem] '>
        {beautyAndSkin.map((item)=>
        <div class="max-w-sm bg-white border border-gray-200 rounded-2xl shadow dark:bg-gray-800 dark:border-gray-700">
        <a href="#">
            <img class="p-8 rounded-t-lg h-[50%] w-[100%] object-cover" src={item.image} alt="product image" />
        </a>
        <div class="px-5 pb-5">
            <a href="#">
                <h5 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white"> quisquam tio exercitationem eum quod itaque nobis molestias explicabo velit sint.</h5>
            </a>
            ...
            <div class="flex items-center justify-between">
                ...
                onClick={handleButtonClick(item)}>Add to cart</div>
            </div>
        </div>
    </div>)}
